...
class Api extends PromiseEventEmitter {
    ...
    async execute(request) {
        ...
        let ret;
        try {
            if (request.format === 'default' && typeof resource.actions[request.action] === 'function') {
                ...
                if (!has(resource.actions[request.action], request.format)
                    || typeof resource.actions[request.action][request.format] !== 'function') {
                    throw new RequestError(`Invalid format "${request.format}" for action "${request.action}"`);
                }
                ret = await (resource.actions[request.action][request.format])(request, response);
            }
