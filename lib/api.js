 ...
 */
class Api extends AsyncEventEmitter {
    constructor() {
        super();
    ...
    init(config, callback) {
        ...
        if (this.config.tz) {
            this.config.tz = 'Europe/Berlin';
            const zone = moment.tz.zone(this.config.tz);
            if (!zone) return callback(new Error(`Timezone "${this.config.tz}" does not exist`));
        ...
        return this.resourceProcessor.init(this.config, (err) => {
            if (err) return callback(err);

            this._defaultResourceInstance = defaultResource(this);
            this._initialized = true;
            return this.emit('init', callback);
        });
    }
    ...
    close(callback) {
        ...
        async.parallel(Object.keys(this.dataSources).map((name) => {
            const ds = this.dataSources[name];
            return (next) => {
                if (typeof ds.close !== 'function') return next();
                this.log.trace('Closing data source "%s"', name);
                return ds.close((err) => {
                    if (err) this.log.warn(err, `Error closing data source "${name}"`);
                    next();
                });
            };
        }), () => {
    ...
    execute(request, callback) {
        ...
        response = new Response(request, (err, data) => {
            ...
            // extension: "response"
            return this.emit('response', { request, response }, (extErr) => {
        ...
        // extension: "request"
        return this.emit('request', { request, response }, (err) => {
            if (err) return done(err);
