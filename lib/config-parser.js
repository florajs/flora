...
var requestParser = require('flora-request-parser');
...
module.exports = function configParser(resourceConfigs, dataSources) {
    ...
    for (resourceName in resourceConfigs) {
        ...
        if (resourceConfigs[resourceName].config) processNode(resourceConfigs[resourceName].config, context);
    }
    ...
    for (resourceName in resourceConfigs) {
        ...
        if (resourceConfigs[resourceName].config) resolveRelations(resourceConfigs[resourceName].config, context);
    }
};
 ...
 * Example:
 * - "datetime"
 * - "datetime(tz=Europe/Berlin)"
 * - "datetime(tz=Europe/Berlin;foo=bar)"
 *
 * @private
 */
function parseStoredType(value, context) {
    ...
    var mainType = result[1];
    var options = {};
    if (result[2]) {
        var regexOptions = /^(\w+)=(.*)/;
        result[3].split(';').forEach(function (o) {
            var resultOptions = o.match(regexOptions);
            if (!resultOptions) throw new ImplementationError('Invalid storedType "' + value + '"');
            options[resultOptions[1]] = resultOptions[2];
        });
    }

    return {type: mainType, options: options};
}
...
function resolveRelations(attrNode, context) {
    if (attrNode.dataSources || attrNode.resource) {
        ...
        if (attrNode.resource) {
            childResource = context.resourceConfigs[attrNode.resource].config; // FIXME: check for key
            if (!childResource) {
        ...
        if (parentResource) {
            ...
            if (attrNode.joinVia) {
                var joinViaDataSource = attrNode.dataSources[attrNode.joinVia];
