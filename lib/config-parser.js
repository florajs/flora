...
function checkWhitelist(str, whitelist, context) {
    if (whitelist.indexOf(str) === -1) {
        throw new ImplementationError('Invalid "' + str + '" (allowed: ' + whitelist.join(', ') + ')' + context.errorContext);
    }
    return str;
}
...
function parseType(type, context) {
    return checkWhitelist(type, ['string', 'int', 'float', 'boolean', 'date', 'datetime', 'time', 'raw', 'object', 'json'], context);
}
...
function parseFilter(filter, context) {
    ...
    return parseList(filter, ['like', 'equal', 'notEqual', 'greater', 'greaterOrEqual', 'less', 'lessOrEqual'], context);
}
...
function parseSubFilters(subFilters, context) {
    subFilters.forEach(subFilter => parseNode(subFilter, {
...
function handleResourceContext(attrNode, context) {
    ...
    if (attrNode.resource) {
        if ('subFilters' in attrNode) {
            throw new ImplementationError('Adding subFilters for included sub-resource is not allowed' + context.errorContext);
        }
        if ('primaryKey' in attrNode) {
            throw new ImplementationError('Overwriting primaryKey for included sub-resource is not allowed' + context.errorContext);
        }
    ...
    if (attrNode.dataSources) {
        Object.keys(attrNode.dataSources).forEach((dataSourceName) => {
            ...
            if (dataSource.inherit) {
                if (!attrNode.resource) {
                    throw new ImplementationError(`DataSource "${dataSourceName}" is defined as "inherit" but has no included resource`);
                }
            ...
            if (!dataSource.type && !dataSource.inherit) {
                throw new ImplementationError(`DataSource "${dataSourceName}" misses "type" option${context.errorContext}`);
            }
    ...
    if (attrNode.joinVia) {
        if (!attrNode.dataSources[attrNode.joinVia]) {
            ...
            if (!dataSource.joinParentKey) {
                throw new ImplementationError(`DataSource "${lastDataSourceName}" misses "joinParentKey" option` + context.errorContext);
            }

            if (!dataSource.joinChildKey) {
                throw new ImplementationError(`DataSource "${lastDataSourceName}" misses "joinChildKey" option` + context.errorContext);
            }
        }
    }
}
...
function handleAttributeContext(attrNode, context) {
    if (!attrNode.type) attrNode.type = 'string';
    ...
    if (attrNode.map) {
        Object.keys(attrNode.map).forEach((mappingName) => {
            ...
            Object.keys(mapping).forEach((dataSourceName) => {
                if (!context.dataSourceAttributes[dataSourceName]) {
                    throw new ImplementationError(`Unknown DataSource "${dataSourceName}" in map${context.errorContext}`);
                }
        ...
        if ('value' in attrNode) {
            throw new ImplementationError('Static "value" in combination with "map" makes no sense' + context.errorContext);
        }
    }
}
            ...
            if (attrNode.joinVia) {
                ...
                if (attrNode.parentKey.length !== joinViaDataSource.joinParentKey.length) {
                    throw new ImplementationError('Composite key length of parentKey (' + attrNode.parentKey.length +
                        ') does not match joinParentKey length (' + joinViaDataSource.joinParentKey.length + ') ' +
                        'of DataSource "' + attrNode.joinVia + '"' + context.errorContext);
                }
                if (attrNode.childKey.length !== joinViaDataSource.joinChildKey.length) {
                    throw new ImplementationError('Composite key length of childKey (' + attrNode.childKey.length +
                        ') does not match joinChildKey length (' + joinViaDataSource.joinChildKey.length + ') ' +
                        'of DataSource "' + attrNode.joinVia + '"' + context.errorContext);
                }
...
function prepareDataSources(attrNode, context) {
    Object.keys(attrNode.dataSources).forEach((dataSourceName) => {
        ...
        dataSourceAttributes = dataSourceAttributes.filter((value, index, self) =>
