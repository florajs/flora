...
function resolvePrimaryKey(attrNode, context) {
    ...
    attrNode.resolvedPrimaryKey = resolveKey(attrNode.primaryKey, attrNode, neededDataSources, context);
...
function prepareDataSources(attrNode, context) {
    ...
    for (dataSourceName in attrNode.dataSources) {
        ...
        if (dataSource.joinParentKey && dataSource.joinChildKey) {
            var errorContext = context.errorContext;
            context.errorContext = ' in joinParentKey' + errorContext;
            dataSource.resolvedJoinParentKey =
                resolveKey(dataSource.joinParentKey, attrNode, [dataSourceName], context);
            dataSource.resolvedJoinParentKey = dataSource.resolvedJoinParentKey[dataSourceName];

            context.errorContext = ' in joinChildKey' + errorContext;
            dataSource.resolvedJoinChildKey =
                resolveKey(dataSource.joinChildKey, attrNode, [dataSourceName], context);
...
function resolveRelations(attrNode, context) {
    if (attrNode.dataSources || attrNode.resource) {
        ...
        if (parentResource) {
            ...
            attrNode.resolvedParentKey = resolveKey(attrNode.parentKey, parentResource, ['primary'], context);

            context.errorContext = ' in childKey' + errorContext;
            attrNode.resolvedChildKey = resolveKey(attrNode.childKey, childResource, ['primary'], context);
            context.errorContext = errorContext;
        }
 ...
 * @private
 */
function resolveKey(key, attrNode, neededDataSources, context) {
    ...
    key.forEach(function (keyAttrPath) {
        ...
        neededDataSources.forEach(function (neededDataSource) {
