...
var ImplementationError = require('./errors').ImplementationError;
...
module.exports = function configParser(resourceConfigs, dataSources) {
    ...
    var context = {
        ...
        parentDataSourceAttributes: null, // ... reference to parent to handle parentKey
...
function processNode(attrNode, context) {
    ...
    if (attrNode.dataSources || attrNode.resource || isMainResource) {
        ...
        context.subAttrPath = [];
        context.parentDataSourceAttributes = context.dataSourceAttributes;
    ...
    if (attrNode.dataSources) {
        ...
        prepareDataSources(attrNode.dataSources, context);
    }
}
...
function handleResourceContext(attrNode, context) {
    ...
    if (attrNode.dataSources) {
        for (dataSourceName in attrNode.dataSources) {
            ...
            if (dataSource.joinParentKey) {
                ...
                dataSource.joinParentKey = parseAttributeList(dataSource.joinParentKey, context);

                dataSource.joinParentKey.forEach(function (attribute) {
                    context.dataSourceAttributes[dataSourceName].push(attribute);
                });
            }

            if (dataSource.joinChildKey) {
                context.errorContext = ' in joinChildKey' + errorContext;
                dataSource.joinChildKey = parseAttributeList(dataSource.joinChildKey, context);

                dataSource.joinChildKey.forEach(function (attribute) {
                    context.dataSourceAttributes[dataSourceName].push(attribute);
                });
            }
    ...
    if (attrNode.parentKey) {
        for (dataSourceName in attrNode.parentKey) {
            if (!context.parentDataSourceAttributes[dataSourceName]) {
                throw new ImplementationError(
                    'Unknown DataSource "' + dataSourceName + '" in parentKey' + context.errorContext);
            }

            attrNode.parentKey[dataSourceName].forEach(function (attribute) {
                context.parentDataSourceAttributes[dataSourceName].push(attribute);
            });
        }
    }

    if (attrNode.childKey) {
        for (dataSourceName in attrNode.childKey) {
            if (!context.dataSourceAttributes[dataSourceName]) {
                throw new ImplementationError(
                    'Unknown DataSource "' + dataSourceName + '" in childKey' + context.errorContext);
            }

            attrNode.childKey[dataSourceName].forEach(function (attribute) {
                context.dataSourceAttributes[dataSourceName].push(attribute);
            });
        }
    }
...
function resolvePrimaryKey(attrNode, context) {
    ...
    attrNode.primaryKey.forEach(function (primaryKeyAttrPath) {
        ...
        // enable "equal" filter for non-composite primary keys by default:
...
function getLocalAttribute(path, attrNode, context) {
    path.forEach(function (attributeName) {
        if (!attrNode.attributes[attributeName]) {
            throw new ImplementationError(
 ...
 * @private
 */
function prepareDataSources(dataSources, context) {
...
/**
 * Parses "{primary}", "id,context", "primary:id,context;fulltextSearch:testId,ctx".
 *
 * @private
 */
function parseRelationKey(relationKey, context) {
    ...
    if (relationKey === '{primary}') {
        ...
        relationKey.split(';').forEach(function (part) {
            var dataSource;
            ...
            parsed[dataSource] = parseAttributeList(parts[0], context);
        });
    }
    return parsed;
}
...
/**
 * Parses 'string', 'int', 'float', 'boolean', 'date', 'datetime', 'time'.
 *
 * @private
 */
function parseType(type, context) {
    return checkWhitelist(type, ['string', 'int', 'float', 'boolean', 'date', 'datetime', 'time'], context);
}
...
/**
 * Parses "id", "id,context".
 *
 * @private
 */
function parseAttributeList(attrList, context) {
    var parsed = attrList.split(',');
    parsed.forEach(function (item) {
        checkIdentifier(item, context);
    });
    return parsed;
