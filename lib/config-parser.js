    ...
    if (attrNode.dataSources) {
        for (dataSourceName in attrNode.dataSources) {
            ...
            if (dataSource.joinParentKey) {
                ...
                if (!dataSource.joinChildKey) throw new ImplementationError(
                    'DataSource "' + dataSourceName + '" misses "joinChildKey" option' + context.errorContext);
            }
            ...
            if (dataSource.joinChildKey) {
                ...
                if (!dataSource.joinParentKey) throw new ImplementationError(
                    'DataSource "' + dataSourceName + '" misses "joinParentKey" option' + context.errorContext);
            }
        }
    }
        ...
        if (dataSource.joinParentKey && dataSource.joinChildKey) {
            ['joinParentKey', 'joinChildKey'].forEach(function (joinKeyName) {
                ...
                dataSource[joinKeyName + 'Resolved'] = dataSource[joinKeyName].map(function (joinKeyAttrPath) {
                    ...
                    if (!joinKeyAttrNode.map.default[dataSourceName]) throw new ImplementationError(
