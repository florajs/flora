...
function explainDataSourceTree(dst, full) {
    ...
    if (dst.subFilters) {
        explain.subFilters = dst.subFilters.map(
            subFilter => explainDataSourceTree(subFilter, full)
        );
    }
    ...
    if (full) {
        ...
        explain.filter = dst.request.filter
            ? dst.request.filter.map(orFilter => orFilter.map((andFilter) => {
                const operators = {
                    like: '~',
                    equal: '=',
                    notEqual: '!=',
    ...
    if (dst.subRequests) {
        explain.subRequests = dst.subRequests.map(
            subRequest => explainDataSourceTree(subRequest, full)
        );
    }
...
class ResourceProcessor {
    ...
    _initResources() {
        ...
        return Promise.all(Object.keys(resources).map(resourceName => new Promise((resolve) => {
    ...
    async handle(request, response) {
        ...
        if (resolved.deprecated) {
            ...
            if (Array.isArray(resolved.deprecated)) {
                resolved.deprecated.forEach((attribute) => {
                    ...
                    this.api.log.debug({ req: request.httpRequest }, `Attribute "${attribute}" in resource "${request.resource}" is deprecated`);
                });
            }
        }
        ...
        if (results.cursor) {
            ...
            if (response.cursor.limit && response.cursor.totalCount) {
                response.cursor.totalPages = Math.ceil(
                    response.cursor.totalCount / response.cursor.limit
                );
            }
        }
