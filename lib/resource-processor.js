...
ResourceProcessor.prototype.init = function init(config, callback) {
    ...
    configLoader(this.api, {
        ...
        self._initResources(callback);
    });
};

ResourceProcessor.prototype._initResources = function initResources(callback) {
    var self = this;

    this.log.debug('Initializing resources');

    var defaultResourceInstance = require('./default-resource')(self.api);
    var resources = this.resourceConfigs;

    async.parallel(Object.keys(resources).map(function (resourceName) {
        return function (done) {
            self.log.trace('Initializing resource "' + resourceName + '"');

            if (!resources[resourceName].instance) {
                resources[resourceName].instance = defaultResourceInstance;
                // no need for "init" extension for the default resource
                return done();
            }

            var resource = resources[resourceName].instance;
            if (resource.extensions && resource.extensions.init && typeof resource.extensions.init === 'function') {
                self.log.trace('"init" extension (%s)', resourceName);
