...
ResourceProcessor.prototype.handle = function handle(request, response) {
    ...
    async.waterfall([
        ...
        function (resolved, callback) {
            ...
            var time = timer.start('preExecute');
            self.api.emit('preExecute', {
                request: request,
                dataSourceTree: resolved.dataSourceTree
            }, function (err) {
                time.end();
        ...
        function (resolved, rawResults, callback) {
            ...
            var time = timer.start('postExecute');
            self.api.emit('postExecute', {
                request: request,
