        ...
        Object.keys(dependency.select).forEach((subAttrName) => {
            ...
            if (!req.select[subAttrName]) {
                ...
                if (subAttrNode.depends) { // resolve dependency of dependency:
                    mergeRequest(context.refRequest, { select: subAttrNode.depends },
                        context.refAttrNode, context.refContext);
                }
            } else {
                mergeRequest(req.select[subAttrName], dependency.select[subAttrName],
                    subAttrNode, subContext);
            }
        });
    }
}
...
function resolveDependencies(req, attrNode, context) {
    ...
    if (attrNode.depends) {
        ...
        mergeRequest(context.refRequest, { select: attrNode.depends },
            context.refAttrNode, context.refContext);
    }
