...
function mergeSubResource(attrNode, subResourceConfig, context) {
    ...
    Object.keys(subResourceConfig).forEach((optionName) => {
        if (optionName === 'attributes') {
            ...
            const newDataSources = _.clone(subResourceConfig.dataSources);
...
function getAttribute(path, attrNode, context) {
    path.forEach((attributeName, i) => {
        if (!(attrNode.attributes && attrNode.attributes[attributeName])) {
            if (attrNode._attrsRefs) {
                ...
                attrNode._attrsRefs.forEach((attrsRef) => {
                    ...
                    if (subAttrNode) {
                        throw new ImplementationError(
                            `Cannot overwrite attribute "${attributeName}" ` +
                            `in "${context.attrPath.join('.')}"`);
                    }
        ...
        if (attrNode.resource) {
            const subContext = _.clone(context);
...
function collectDependencies(req, attrNode, context, dependencies) {
    dependencies = dependencies || [];
    if (attrNode.depends) dependencies.push(attrNode.depends);
    ...
    if (req.select) {
        ...
        Object.keys(req.select).forEach((subAttrName) => {
            ...
            collectDependencies(req.select[subAttrName], subAttrNode, subContext, dependencies);
        });
    }
...
function resolveDependencies(req, attrNode, context) {
    const dependencies = collectDependencies(req, attrNode, context);

    // merge dependencies into request:
    dependencies.forEach(dependency => mergeRequest(req, { select: dependency }));
}
                    ...
                    if (subFilter.rewriteTo) {
                        ...
                        subResourceAttrNodes.forEach((subResourceAttrNode) => {
                            ...
                            subResourceAttrNode.childKey.forEach((childKeyAttrPath) => {
                                const childKeyAttrNode =
                                    getAttribute(childKeyAttrPath, subResourceAttrNode, context);
                            ...
                            if (subResourceAttrNode.joinVia) {
                                const joinViaDataSource =
                                    subResourceAttrNode.dataSources[subResourceAttrNode.joinVia];
                                ...
                                joinViaDataSource.joinParentKey.forEach((joinParentKeyAttrPath) => {
                                    const joinParentKeyAttrNode =
                                        getAttribute(joinParentKeyAttrPath, subResourceAttrNode,
                                        context);
    ...
    if (attrNode.dataSources) {
        ...
        if (context.resourceTree) {
            ...
            attrNode.parentKey.forEach((parentKeyAttrPath) => {
                const parentKeyAttrNode =
                    getAttribute(parentKeyAttrPath, context.resourceTree.attrNode, context);
...
function selectPrimaryDataSource(resourceTree) {
    ...
    if (resourceTree.primaryName) {
        ...
        primaryName = Object.keys(resourceTree.dataSources).find(dataSourceName =>
            resourceTree.dataSources[dataSourceName].searchable);
...
function resolveResourceTree(resourceTree, parentDataSourceName) {
    ...
    Object.keys(dataSources).forEach((dataSourceName) => {
        dataSources[dataSourceName] = _.clone(resourceTree.dataSources[dataSourceName]);
    });
    if (resourceTree.attrNode) {
