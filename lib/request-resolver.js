                    ...
                    if (newDataSources[dataSourceName]) {
                        if (attrNode.dataSources[dataSourceName].inherit) {
                            if (attrNode.dataSources[dataSourceName].inherit === 'inherit') {
                                ...
                                newDataSources[dataSourceName] = attrNode.dataSources[
                                    dataSourceName
                                ];
                            }
                        } else {
...
function resolveIncludes(attrNode, context) {
    ...
    while (attrNode.resource) {
        ...
        if (!context.resourceConfigs[attrNode.resource]
            || !context.resourceConfigs[attrNode.resource].config) {
        ...
        if (!(attrNode.attributes && attrNode.attributes[attributeName])) {
            if (attrNode._attrsRefs) {
                ...
                attrNode._attrsRefs.forEach((attrsRef) => {
                    ...
                    if (subAttrNode) {
                        throw new ImplementationError(`Cannot overwrite attribute "${attributeName}" `
                            + `in "${context.attrPath.join('.')}"`);
                    }
            ...
            if (!(attrNode.attributes && attrNode.attributes[attributeName])) {
                throw new RequestError('Unknown attribute '
                    + `"${context.attrPath.concat(path.slice(0, i + 1)).join('.')}" in request`);
            }
        }
    ...
    if (dependency.select) {
        ...
        Object.keys(dependency.select).forEach((subAttrName) => {
            ...
            if (!req.select[subAttrName]) {
                ...
                if (subAttrNode.depends) { // resolve dependency of dependency:
                    mergeRequest(
                        context.refRequest,
...
function processRequestOptions(req, attrNode, context) {
    ...
    if ('filter' in req) {
        let requestFilter = req.filter.map(andFilter => andFilter.map((filter) => {
            ...
            if (subResourceAttrNodes.length > 0) {
                ...
                if (!subFilter) {
                    throw new RequestError(`Can not filter by sub-resource attribute "${filter.attribute.join('.')}"`
                        + (context.attrPath.length > 0 ? ` (in "${context.attrPath.join('.')}")` : ''));
                }
                ...
                if (subFilter.rewriteTo) {
                    ...
                    subResourceAttrNodes.forEach((subResourceAttrNode) => {
                        ...
                        subResourceAttrNode.childKey.forEach((childKeyAttrPath) => {
                            const childKeyAttrNode = getAttribute(
                                childKeyAttrPath, subResourceAttrNode, context
                            );
                        ...
                        if (subResourceAttrNode.joinVia) {
                            const joinViaDataSource = subResourceAttrNode.dataSources[
                                subResourceAttrNode.joinVia
                            ];

                            const resolvedJoinParentKey = {};
                            resolvedJoinParentKey[subResourceAttrNode.joinVia] = joinViaDataSource
                                .resolvedJoinParentKey;
                            const resolvedJoinChildKey = {};
                            resolvedJoinChildKey[subResourceAttrNode.joinVia] = joinViaDataSource
                        ...
                        previousParentFilter.valueFromSubFilter = filterContext
                            .resourceTree.subFilters.length;
        ...
        context.resourceTree.order = order.map((orderPart) => {
            ...
            if (orderedAttrNode.order.indexOf(orderPart.direction) === -1) {
                throw new RequestError([
                    'Attribute "' + orderPart.attribute.join('.') + '" ',
                    context.attrPath.length > 0 ? ' (in "' + context.attrPath.join('.') + '")' : '',
                    'can not be ordered "' + orderPart.direction + '" (allowed: ' + orderedAttrNode.order.join(', ') + ')'
                ].join(''));
            }
    ...
    if ('limit' in req) {
        if (!attrNode.many) {
            throw new RequestError('Invalid limit on a single resource'
                + (context.attrPath.length > 0 ? ` (in "${context.attrPath.join('.')}")` : ''));
        }
        limit = req.limit;
    } else if (attrNode.many) {
        limit = attrNode.defaultLimit
            || attrNode.maxLimit
            || (context.isMainResource ? 10 : null);
    }
    if (attrNode.maxLimit) {
        if (limit === null || limit > attrNode.maxLimit) {
            throw new RequestError(`Invalid limit ${(limit !== null ? limit : 'unlimited')}, maxLimit is ${attrNode.maxLimit}`
                + (context.attrPath.length > 0 ? ` (in "${context.attrPath.join('.')}")` : ''));
        }
    }
    if (limit !== null) {
...
function mapRequestRecursive(req, attrNode, context) {
    ...
    if (attrNode.deprecated && !req.internal) {
        //log.info(new RequestError('Attribute "' + context.attrPath.join('.') + '" is deprecated'));
    }
    ...
    if (attrNode.dataSources) {
        ...
        if (context.resourceTree) {
            ...
            attrNode.parentKey.forEach((parentKeyAttrPath) => {
                const parentKeyAttrNode = getAttribute(
                    parentKeyAttrPath, context.resourceTree.attrNode, context
                );
...
function resolveDataSourceAttributes(resourceTree, dataSources, primaryName) {
    ...
    resourceTree.attributes.forEach(function resolveAttribute(attrInfo) {
        ...
        if (attrInfo.fromDataSource === '#all-selected') {
            ...
            attrInfo.attrNode.selectedDataSource = Object.keys(attrInfo.dataSourceMap)
                .find(dataSourceName => dataSources[dataSourceName]);
        ...
        selectedDataSources.forEach((selectedDataSourceName) => {
            ...
            dataSources[selectedDataSourceName].attributeOptions[attribute] = _.pick(
                attrInfo.attrNode, ['type', 'storedType', 'multiValued', 'delimiter']
            );
        });
...
function resolveDataSourceOptions(resourceTree, dataSources, primaryName) {
    if (resourceTree.filter) {
        dataSources[primaryName].filter = resourceTree.filter.map(andFilter => andFilter.map(
            (filter) => {
...
module.exports = function requestResolver(req, resourceConfigs) {
    ...
    return { resolvedConfig, dataSourceTree };
};
