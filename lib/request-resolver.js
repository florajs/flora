    ...
    if (attrNode.dataSources) {
        ...
        if (attrNode.primaryKey && attrNode.resolvedPrimaryKey) {
            ...
            attrNode.primaryKey.forEach(function (primaryKeyAttrPath) {
                ...
                if (!primaryKeyAttrNode.internal) {
        ...
        if (context.resourceTree) {
            ...
            // resolve parentKey and select from DataSource:
            subResourceTree.parentKey = {};
            attrNode.parentKey = attrNode.parentKey || context.resourceTree.attrNode.primaryKey;
            attrNode.parentKey.forEach(function (parentKeyAttrPath) {
                ...
                for (var dataSourceName in parentKeyAttrNode.map.default) {
                    if (!subResourceTree.parentKey[dataSourceName]) subResourceTree.parentKey[dataSourceName] = [];
                    subResourceTree.parentKey[dataSourceName].push(parentKeyAttrNode.map.default[dataSourceName]);
                }
            });

            // resolve childKey and select from DataSource:
            subResourceTree.childKey = {};
            attrNode.childKey = attrNode.childKey || attrNode.primaryKey;
            attrNode.childKey.forEach(function (childKeyAttrPath) {
                ...
                for (var dataSourceName in childKeyAttrNode.map.default) {
                    if (!subResourceTree.childKey[dataSourceName]) subResourceTree.childKey[dataSourceName] = [];
                    subResourceTree.childKey[dataSourceName].push(childKeyAttrNode.map.default[dataSourceName]);
                }
            });
...
function resolveResourceTree(resourceTree, parentPrimaryName) {
    ...
    if (resourceTree.joinVia) {
        ...
        joinDataSource.attributes = joinDataSource.joinParentKeyResolved.concat(joinDataSource.joinChildKeyResolved);
        joinDataSource.filter = [[{
            attribute: joinDataSource.joinParentKeyResolved.length === 1 ?
                joinDataSource.joinParentKeyResolved[0] : joinDataSource.joinParentKeyResolved,
            operator: 'equal',
            valueFromParentKey: true
        }]];
...
function resolveDataSourceAttributes(resourceTree, dataSources, primaryName) {
    ...
    resourceTree.attributes.forEach(function (attrInfo) {
        ...
        selectedDataSources.forEach(function (dataSourceName) {
            var attribute = attrInfo.dataSourceMap[dataSourceName];
