...
function mapRequestRecursive(req, attrNode, context) {
    ...
    if (attrNode.dataSources) {
        ...
        if (context.resourceTree) {
            ...
            subResourceTree.keyAttributes.push({dataSourceMap: subResourceTree.childKey, attrNode: null});
            ...
            // prepare cast for parentKey (just for delimiter for now):
            if (attrNode.delimiter) {
                if (!context.resourceTree.casts) {
                    context.resourceTree.casts = [];
                }
                context.resourceTree.casts.push({
                    attribute: subResourceTree.parentKey,
                    type: attrNode.type || 'string', // TODO: Normalize "type" in config-parser?
                    delimiter: attrNode.delimiter
                });
            }
...
function resolveResourceTree(resourceTree, parentPrimaryName) {
    ...
    if (resourceTree.casts) {
        dataSourceTree.casts = {};
        resourceTree.casts.forEach(function (cast) {
            if (!cast.attribute[primaryName]) {
                return;
            }

            var attributes = cast.attribute[primaryName];
            ...
            attributes.forEach(function (attribute) {
                delete cast.attribute;
                dataSourceTree.casts[attribute] = cast;
            });
        });
    }
...
function resolveDataSourceAttributes(resourceTree, dataSources, primaryName) {
    ...
    for (dataSourceName in dataSources) {
        ...
        resourceTree.keyAttributes.forEach(function (attrInfo) {
            if (!attrInfo.dataSourceMap[dataSourceName]) {
                throw new ImplementationError('Key attribute mapping failed');
            }
            dataSource.attributes = dataSource.attributes.concat(attrInfo.dataSourceMap[dataSourceName]);
        });
    }

    resourceTree.keyAttributes.forEach(function (attrInfo) {
        if (!attrInfo.attrNode) return;
    ...
    resourceTree.attributes.forEach(function (attrInfo) {
        ...
        if (attribute === null) {
            throw new ImplementationError('Attribute mapping failed');
        }

        if (Array.isArray(attribute)) { // arrays for composite keys cannot be resolved earlier
            dataSources[dataSourceName].attributes = dataSources[dataSourceName].attributes.concat(attribute);
        } else {
