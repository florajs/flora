...
function mergeSubResource(attrNode, subResourceConfig, context) {
    ...
    for (optionName in subResourceConfig) {
        if (optionName === 'attributes') {
            ...
            newDataSources = _.clone(subResourceConfig.dataSources);
            ...
            if (typeof subResourceConfig[optionName] === 'object') {
    ...
    path.forEach(function (attributeName, i) {
        if (!(attrNode.attributes && attrNode.attributes[attributeName])) {
            if (attrNode._attrsRefs) {
                var subAttrNode = null;
                attrNode._attrsRefs.forEach(function (attrsRef) {
                    ...
                    var attrRef = attrsRef[attributeName];

                    subAttrNode = {};
                    for (var optionName in attrRef) {
                        if (optionName === 'attributes') {
                            ...
                            if (typeof attrRef[optionName] === 'object') {
...
function resolveResourceTree(resourceTree, parentDataSourceName) {
    ...
    if (resourceTree.joinVia) {
        var joinDataSource = dataSources[resourceTree.joinVia];
