...
function mapRequestRecursive(req, attrNode, context) {
    ...
    if (attrNode.dataSources) {
        ...
        if (context.resourceTree) {
            ...
            // default limit (may be overwritten by processRequestOptions)
            if (!req.limit && attrNode.defaultLimit) {
                req.limit = attrNode.defaultLimit;
            }

            // default order (may be overwritten by processRequestOptions)
            if (!req.order && attrNode.defaultOrder) {
                // TODO:
                // There is no error handling if the defaultOrder attribute cannot be ordered.
                // This should be an ImplementationError instead of RequestError
                req.order = attrNode.defaultOrder;
            }

            // maximum limit value. Is set when no limit is there
            // if limit in request is higher, a RequestError is thrown
            if (attrNode.maxLimit) {
                if (!req.limit) req.limit = attrNode.maxLimit;
                if (req.limit > attrNode.maxLimit) {
                    throw new RequestError(
                        'Invalid limit ' + req.limit + ' on "' + context.attrPath.join('.') +
                        '", maxLimit is ' + attrNode.maxLimit);
                }
            }
