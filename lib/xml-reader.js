...
function parse(node) {
    ...
    return Object.assign(
        ...
        // handle basic data attributes
        elementNodes
            .filter(node => !node.namespaceURI)
            .reduce((cfg, node) => {
        ...
        // handle data source nodes
        floraNodes
            .filter(node => node.localName === 'dataSource')
            .reduce((cfg, node) => {
        ...
        // handle sub-filters nodes
        floraNodes
            .filter(node => node.localName === 'subFilter')
            .reduce((cfg, node) => {
