...
function executeDst(api, request, dst) {
    ...
    .then((allSubFilterResults) => {
        ...
        allSubFilterResults.forEach((subFilterResults) => {
            const subFilterResult = subFilterResults[0]; // only main result from each subFilter
            const parentKey = subFilterResult.parentKey.join('-');
        ...
        for (const parentKeyIdx in parentValues) {
            for (let i = 0; i < parentValues[parentKeyIdx].length; i++) {
        ...
        dst.request.filter.forEach((filter) => {
            ...
            filter.forEach((filterPart) => {
                ...
                const attributeIdx = Array.isArray(filterPart.attribute)
                ...
                parentValues[attributeIdx].forEach((pv) => {
                    if (dst._isEmpty) return;
                    ...
                    const filterPartNew = _.clone(filterPart);

                    if (!Array.isArray(filterPart.attribute)) {
                        filterPartNew.value = pv;
                    } else {
                        filterPartNew.value = [];
                        for (let j = 0; j < filterPart.attribute.length; j++) {
                ...
                if (Array.isArray(filterPart.attribute)) {
                    for (let j = 0; j < filterPart.attribute.length; j++) {
                        ...
                        if (Array.isArray(filterPart.value[j])) {
                            filterPart.value[j] = filterPart.value[j].map(valuePart =>
                                cast(valuePart, storedType));
                        } else {
    ...
    .then((mainResults) => {
        ...
        for (var idx in mainResults.data) {
            ...
            for (var key in mainResults.data[idx]) {
                ...
                mainResults.data[idx][key] =
                    cast(mainResults.data[idx][key], dst.attributeOptions[key]);
            }
        }
