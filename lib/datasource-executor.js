            ...
            dst.request.filter.forEach(function (f) {
                ...
                f.forEach(function (ff) {
                    ...
                    if (dst.attributeOptions &&
                        dst.attributeOptions[ff.attribute] &&
                        dst.attributeOptions[ff.attribute].storedType) {
                        if (Array.isArray(ff.value)) {
                            ff.value = ff.value.map(function (value) {
                                return cast(value, dst.attributeOptions[ff.attribute].storedType);
                            });
                        } else {
                            ff.value = cast(ff.value, dst.attributeOptions[ff.attribute].storedType);
                        }
                    }
        ...
        .then(function transformMainResults(mainResults) {
            ...
            for (var idx in mainResults.data) {
                ...
                for (var key in mainResults.data[idx]) {
                    ...
                    mainResults.data[idx][key] = cast(mainResults.data[idx][key], dst.attributeOptions[key].type);
                }
            }
