 ...
 */
module.exports = function execute(api, dst, callback) {
    executeDst(api, dst)
 ...
 * @private
 */
function processSubFilters(api, dst) {
    var subFilters = [];
    if (dst.subFilters) {
        subFilters = dst.subFilters.map(function (subFilter) {
            return executeDst(api, subFilter);
        });
    }
...
function executeDst(api, dst) {
    ...
    if (resource && resource.extensions && resource.extensions.preExecute) {
        ...
        resource.extensions.preExecute(dst);
    }

    var finalResults;

    return processSubFilters(api, dst)
        ...
        .then(function prepareSubRequests(mainResults) {
            ...
            var subRequests = dst.subRequests.map(function (subRequest) {
                ...
                return executeDst(api, subRequest);
            });
        ...
        .then(function invokePostExecute(results) {
            ...
            if (resource && resource.extensions && resource.extensions.postExecute) {
                ...
                resource.extensions.postExecute(results);
            }
