...
function executeDst(api, request, dst) {
    ...
    return (new Promise(function (resolve) {
        ...
        .then(function handleSubFilterResults(allSubFilterResults) {
            /*
             * allSubFilterResults:
             * Results from all subFilters.
             * - Outer array: foreach subFilter
             * - Inner array: list of results per subFilter request. As there are no subRequests in subFilters, the length is 1
             */

            //api.log.warn(allSubFilterResults); // FIXME
            if (!dst.request.filter) return true;

            // Build filter array for the main request
            var parentValues = {};
            allSubFilterResults.forEach(function (subFilterResults) {
                var subFilterResult = subFilterResults[0]; // only main result from each subFilter
                ...
                parentValues[parentKey].push(subFilterResult.data.map(function (ff) {
                    // TODO: handle single-keyed parentValues more efficient
             ...
             *     }
             * case 2: single key:
             *    TODO: optimize, so for single-keyed parentValues flatten the inner array
             */
            api.log.warn(parentValues, 'parentValues'); // FIXME
            ...
            for (var parentKeyIdx in parentValues) {
                for (var i = 0; i < parentValues[parentKeyIdx].length; i++) {
                    // TODO: handle single-keyed parentValues more efficient
                    parentValues[parentKeyIdx][i] = _.uniqWith(parentValues[parentKeyIdx][i], _.isEqual);
                }
            }
            ...
            dst.request.filter.forEach(function (filter) {
                ...
                filter.forEach(function (filterPart) {
                    ...
                    parentValues[attributeIdx].forEach(function (pv) {
                        ...
                        // TODO: handle single-keyed parentValues more efficient (pv.isArray...)
            ...
            //console.log('requestFilter', require('util').inspect(requestFilter, null, 10));
        ...
        .then(function transformMainResults(mainResults) {
             ...
             * }
             */
            //console.log('mainResults', require('util').inspect(mainResults, null, 10));
            ...
            var subRequests = dst.subRequests.map(function (subRequest) {
                ...
                if (subRequest.parentKey.length === 1 &&
                    ...
                    parentValues.forEach(function (parentValue) {
                        ...
                        Array.prototype.push.apply(newParentValues, parentValue[0].map(function (value) {
                            return [value];
                        }));
                    });
                ...
                parentValues = _.uniqWith(parentValues, _.isEqual);
