    ...
    return (new Promise(function (resolve) {
        ...
        .then(function handleSubFilterResults(allSubFilterResults) {
            ...
            allSubFilterResults.forEach(function (subFilterResults) {
                ...
                var pvs = [];
                for (var i = 0; i < subFilterResult.data.length; i++) {
                    if (subFilterResult.parentKey.length === 1) {
                        if (subFilterResult.data[i].hasOwnProperty(subFilterResult.childKey[0])) {
                            pvs.push(subFilterResult.data[i][subFilterResult.childKey[0]]);
                        }
                    } else {
                        var partEmpty = false;
                        var part = subFilterResult.childKey.map(function (childKeyPart) {
                            if (!subFilterResult.data[i].hasOwnProperty(childKeyPart)) {
                                partEmpty = true;
                                return null;
                            }
                ...
                parentValues[parentKey].push(pvs);
            });
        ...
        .then(function instantiateResource() {
            var resource = api.getResource(dst.resourceName);
            if (!resource || resource._initialized) return;

            api.log.trace('handle: "init" extension (%s)', dst.resourceName);
        ...
        .then(function executeSubRequests(mainResults) {
            ...
            var subRequests = dst.subRequests.map(function (subRequest) {
                ...
                var flatten = (subRequest.parentKey.length === 1 &&
                    dst.attributeOptions &&
                    dst.attributeOptions[subRequest.parentKey[0]] &&
                    dst.attributeOptions[subRequest.parentKey[0]].multiValued) ? true : false;
                ...
                mainResults.data.forEach(function (mainResult) {
                    ...
                    if (subRequest.parentKey.length === 1) {
                        ...
                        value = subRequest.parentKey.map(function (m) {
                ...
                parentValues = (subRequest.parentKey.length === 1) ? _.uniq(parentValues) : _.uniqWith(parentValues, _.isEqual);
