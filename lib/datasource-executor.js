...
function executeDst(api, request, dst) {
    ...
    return (new Promise(function invoke(resolve, reject) {
        ...
        .then(function handleSubFilterResults(subFilterResults) {
            ...
            for (var parentKey in parentValues) {
                for (var i = 0; i < parentValues[parentKey].length; i++) {
                    parentValues[parentKey][i] = keyUniq(parentValues[parentKey][i]);
                }
            }
        ...
        .then(function executeSubRequests(mainResults) {
            ...
            var subRequests = dst.subRequests.map(function (subRequest) {
                ...
                if (subRequest.parentKey.length === 1 &&
                    ...
                    var newParentValues = [];
                    parentValues.map(function (parentValue) {
                ...
                parentValues = keyUniq(parentValues);
