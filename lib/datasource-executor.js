...
async function executeDst(api, request, dst) {
    ...
    if (dst.request.filter) {
        ...
        */
        ...
        Object.keys(parentValues).forEach((parentKeyIdx) => {
            parentValues[parentKeyIdx] = parentValues[parentKeyIdx].map(
                parentValue => _.uniqWith(parentValue, _.isEqual)
            );
        });
        ...
        const newRequestFilter = requestFilter.filter(
            orFilter => !orFilter.some(andFilter => andFilter.empty)
        );
                ...
                if (Array.isArray(filterPart.attribute)) {
                    filterPart.attribute.forEach((attrPart, index) => {
                        ...
                        if (Array.isArray(filterPart.value[index])) {
                            filterPart.value[index] = filterPart.value[index].map(
                                valuePart => cast.cast(valuePart, storedType)
                            );
                        } else {
                    ...
                    if (Array.isArray(filterPart.value)) {
                        filterPart.value = filterPart.value.map(
                            valuePart => cast.cast(valuePart, storedType)
                        );
                    } else {
    ...
    if (dst.resourceName) {
        ...
        if (resource && resource.extensions && resource.extensions.preExecute) {
            ...
            await resource.extensions.preExecute({
    ...
    [
        ...
        if (has(dst, key)) mainResults[key] = dst[key];
    });

    await new Promise((resolve, reject) => {
        if (dst._isEmpty) return resolve(mainResults);
    ...
    if (dst.resourceName) {
        ...
        if (resource && resource.extensions && resource.extensions.postExecute) {
            ...
            await resource.extensions.postExecute({
                name: dst.dataSourceName,
                request: dst.request,
                floraRequest: request,
