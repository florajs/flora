    ...
    return (new Promise(function (resolve) {
        ...
        .then(function executeSubFilters() {
            ...
            if (dst.subFilters) {
                subFilters = dst.subFilters.map(function (subFilter, i) {
                    subFilter._timer = dst._timer.start('subFilter:' + i);
                    return executeDst(api, request, subFilter).then(function (results) {
                        subFilter._timer.end();
                        return results;
                    });
                });
            }
        ...
        .then(function executeMainRequest() {
            ...
            return new Promise(function (resolve, reject) {
                ...
                dst.request._timer = dst._timer.start(dst.attributePath.join('.') + ':' + dst.dataSourceName + '#dataSourceRequest');
                ds.process(dst.request, function (err, rows) {
                    dst.request._timer.end();
        ...
        .then(function executeSubRequests(mainResults) {
            ...
            var subRequests = dst.subRequests.map(function (subRequest) {
                ...
                subRequest._timer = dst._timer.start(subRequest.attributePath.join('.') + ':' + subRequest.dataSourceName);
                return executeDst(api, request, subRequest).then(function (results) {
                    subRequest._timer.end();
                    return results;
                });
            });
