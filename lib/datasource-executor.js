...
module.exports = function execute(api, request, dst, callback) {
    if (!request._status) request._status = new Status();
...
function executeDst(api, request, dst) {
    ...
    return (new Promise(function (resolve) {
        ...
        .then(function executeSubFilters() {
            var subFilters = [];
            if (dst.subFilters) {
                subFilters = dst.subFilters.map(function (subFilter, i) {
                    subFilter._profiler = dst._profiler.child('subFilter:' + i);
                    return executeDst(api, request, subFilter).then(function (results) {
                        subFilter._profiler.end();
                        return results;
                    });
                });
            }
                ...
                filter.forEach(function (filterPart) {
                    ...
                    if (Array.isArray(filterPart.attribute)) {
                        for (var j = 0; j < filterPart.attribute.length; j++) {
                            ...
                            if (Array.isArray(filterPart.value[j])) {
                                filterPart.value[j] = filterPart.value[j].map(function (valuePart) {
                                    return cast(valuePart, {type: storedType});
                                });
                            } else {
                                filterPart.value[j] = cast(filterPart.value[j], {type: storedType});
                            }
                        }
                    } else {
                        ...
                        if (Array.isArray(filterPart.value)) {
                            ...
                            filterPart.value = cast(filterPart.value, {type: storedType});
                        }
                    }
                });
            });
        ...
        .then(function executeMainRequest() {
            ...
            ['attributePath', 'dataSourceName', 'childKey', 'parentKey',
                'multiValuedParentKey', 'uniqueChildKey'].forEach(function (key) {
            ...
            return new Promise(function (resolve, reject) {
                ...
                dst.request._status = request._status.addChild('dataSourceRequests');
                dst.request._status.set('name', dst.attributePath ?
                    dst.attributePath.join('.') + ':' + dst.dataSourceName : '');
                dst.request._status.set('type', dst.request.type);

                dst.request._profiler = dst._profiler.child((dst.attributePath ?
                    dst.attributePath.join('.') + ':' + dst.dataSourceName : '') +
                    '#dataSourceRequest');
        ...
        .then(function executeSubRequests(mainResults) {
            ...
            var subRequests = dst.subRequests.map(function (subRequest) {
                ...
                subRequest._profiler = dst._profiler.child((subRequest.attributePath ?
                    subRequest.attributePath.join('.') + ':' + subRequest.dataSourceName : ''));
                return executeDst(api, request, subRequest).then(function (results) {
                    subRequest._profiler.end();
                    return results;
                });
            });
