...
module.exports = async function configLoader(api, options) {
    ...
    // parse all configs
    await Promise.all(Object.keys(resources).map((resourceName) => {
        ...
        return parseConfig(file).then((config) => {
    ...
    await Promise.all(Object.keys(resources).map((resourceName) => {
        ...
        return new Promise((resolve /* , reject */) => {
            ...
            resources[resourceName].instance = require(
                resources[resourceName].instanceFile
            )(api);
