...
module.exports = function resultBuilder(api, request, rawResults, resolvedConfig) {
    ...
    if (!resolvedConfig.many) {
        if (rawResult.data.length <= 0) {
            throw new NotFoundError('Item' + (request.id ? ' "' + request.id + '"' : '') + (request.resource ? ' (in resource "' + request.resource + '")' : '') + ' not found');
        }
    ...
    rawResults.forEach(function (rawResult, resultId) {
        ...
        if (rawResult.childKey) {
            ...
            rawResult.data.forEach(function (row, i) {
                ...
                var key;
                if (keyAttr) {
                    key = '' + dereferenceKeyAttr(keyAttr); // speed up non-composite keys
                } else {
                    key = rawResult.childKey.map(dereferenceKeyAttr).join(keySeparator);
                }
    ...
    for (attrName in parentAttrNode.attributes) {
        ...
        if (attrNode.attributes) {
            ...
            if (attrNode.dataSources) {
                ...
                var rawResult, keys, keyIsNull = true;
                var parentKeyRow = row;
                ...
                if (parentKeyRow) {
                    ...
                    keys = rawResult.parentKey.map(function (keyAttr) {
