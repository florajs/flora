 ...
 */
module.exports = function resultBuilder(api, rawResults, resolvedConfig) {
...
function getAttribute(path, attrNode) {
    path.forEach(function (attrName) {
        if (!attrNode.attributes[attrName]) {
...
function buildItem(parentAttrNode, row, context) {
    ...
    if (context.primaryKey === null) {
        ...
        var indexKey = resAttrNode.resolvedPrimaryKey || resAttrNode.childKey;
        context.primaryKey = indexKey[resAttrNode.primaryDataSource].map(function (keyAttr) {
            if (typeof row[keyAttr] === 'undefined') {
        ...
        for (var dataSourceName in resAttrNode.dataSources) {
            ...
            if (typeof secondaryResult.indexedData[context.primaryKey] === 'undefined') {
                throw new DataError('Secondary DataSource "' + dataSourceName + '" of ' +
                    '"' + (context.attrPath.length > 0 ? context.attrPath.join('.') : '{root}') + '": ' +
    ...
    for (attrName in parentAttrNode.attributes) {
        ...
        if (attrNode.attributes) {
            ...
            if (attrNode.dataSources) {
                // resource context:
                var rawResult = getPrimaryResult(attrNode, context);
                ...
                if (!subRow) {
                    throw new DataError('Row with child key "' + key + '" not found ' +
                        'in sub-resource "' + subContext.attrPath.join('.') + '" ' +
    ...
    if (resource && resource.events && resource.events.item) {
        if (resource.events.item.length > 1) {
            context.api.log.warn('"item" function of "%s" resource accepts 2 arguments instead of 1. ' +
                'Asynchronous "item" extensions are not supported yet.',
                parentAttrNode.resourceName);
        }
        context.api.log.trace('handle: "item" extension (%s)', parentAttrNode.resourceName);
        resource.events.item(item);
    }
...
function buildAttribute(attrNode, row, context) {
    ...
    // type casting:
    if (value === null) {
        // always pass through 'null' ...
    } else if (!attrNode.type || attrNode.type === 'string') {
        value = '' + value;
    } else if (attrNode.type === 'int') {
        value = parseInt(value, 10);
    } else if (attrNode.type === 'float') {
        value = parseFloat(value);
    } else if (attrNode.type === 'boolean') {
        value = value === '0' ? false : !!value;
    } else if (attrNode.type === 'date') {
        value = (new Date(value)).toISOString();
    } else if (attrNode.type === 'datetime') {
        value = (new Date(value)).toISOString();
    } else if (attrNode.type === 'time') {
        value = (new Date(value)).toISOString();
    } else {
        // TODO: error?
    }
