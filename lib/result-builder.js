            ...
            rawResult.data.forEach((row, i) => {
                function dereferenceKeyAttr(keyAttrib) {
                    ...
                    if (keyVal === undefined) {
                        ...
                        throw new DataError(
                            `Result-row ${i} of "${attrPath}" (DataSource "${rawResult.dataSourceName}") misses child key attribute "${keyAttrib}"`
                        );
                    }
                    return keyVal;
                }
...
function resolvePrimaryKey(row, context) {
    ...
    function dereferenceKeyAttr(keyAttr) {
        ...
        if (keyVal === undefined) {
            ...
            throw new DataError(
                `Result-row of "${attrPath}" (DataSource "${resAttrNode.primaryDataSource}") misses primary key attribute "${keyAttr}"`
            );
        }
        return keyVal;
    }
...
function buildItem(parentAttrNode, row, context) {
    ...
    Object.keys(parentAttrNode.attributes).forEach((attrName) => {
        ...
        if (attrNode.attributes) {
            ...
            if (attrNode.dataSources) {
                ...
                if (attrNode.cursor) {
                    let cursor = { totalCount: rawResult ? rawResult.totalCount : null, ...attrNode.cursor };
                    cursor.totalPages =
                        cursor.limit && cursor.totalCount !== null ? Math.ceil(cursor.totalCount / cursor.limit) : null;
                    context.cursors[subContext.attrPath.join('.')] = cursor;
                }
    ...
    if (parentAttrNode.extensions && parentAttrNode.extensions.item) {
        let extensionList = parentAttrNode.extensions.item;
        let ev = {
            item,
            row,
            ...
            buildItem: (attributes) => {
                ...
                if (subContext.selectedInternalLevel > maxRecursionLevel) {
                    ...
                    throw new ImplementationError(`item-extension/buildItem: Recursion depth for "${attrStr}" too big`);
                }
        ...
        extensionList.forEach((extension) => extension(ev));
    }
...
module.exports = function resultBuilder(api, request, rawResults, resolvedConfig) {
    ...
    if (!resolvedConfig.many) {
        ...
        results.data = buildItem(resolvedConfig, rawResult.data[0], context);
    } else {
        results.cursor = { totalCount: rawResult.totalCount };
        results.data = rawResult.data.map((row) => buildItem(resolvedConfig, row, context));
    }

    if (Object.keys(context.cursors).length > 0) {
        results.cursor = results.cursor || {};
        results.cursor.subResources = context.cursors;
    }
