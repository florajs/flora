        ...
        if (rawResult.childKey) {
            ...
            rawResult.data.forEach((row, i) => {
                function dereferenceKeyAttr(keyAttrib) {
                    ...
                    if (keyVal === undefined) {
                        const attrPath = (rawResult.attributePath.length > 0) ? rawResult.attributePath.join('.') : '{root}';
                        throw new DataError(`Result-row ${i} of "${attrPath}" (DataSource "${rawResult.dataSourceName}") misses child key attribute "${keyAttr}"`);
                    }
                    return keyVal;
                }
                ...
                if (!rawResult.uniqueChildKey) {
                    ...
                    if (rawResult.indexedData[key]) {
                        const attrPath = (rawResult.attributePath.length > 0) ? rawResult.attributePath.join('.') : '{root}';
                        throw new DataError(`Result-row ${i} of "${attrPath}" (DataSource "${rawResult.dataSourceName}") has duplicate child key "${key}"`);
                    }
...
function resolvePrimaryKey(row, context) {
    ...
    function dereferenceKeyAttr(keyAttr) {
        ...
        if (keyVal === undefined) {
            ...
            throw new DataError(`Result-row of "${attrPath}" (DataSource "${resAttrNode.primaryDataSource}") misses primary key attribute "${keyAttr}"`);
        }
        return keyVal;
    }
    ...
    Object.keys(resAttrNode.dataSources).forEach((dataSourceName) => {
        ...
        const secondaryResult =
            context.rawResults[resAttrNode.dataSources[dataSourceName].resultId];
    ...
    if (attrNode.map) {
        ...
        if (attrNode.selectedDataSource === context.resourceAttrNode.primaryDataSource) {
            ...
            if (typeof context.secondaryRows[attrNode.selectedDataSource] === 'undefined') {
                const attrPath = (context.attrPath.length > 0) ? context.attrPath.join('.') : '{root}';
                throw new ImplementationError(`Secondary-Result for "${attrPath}" (DataSource "${attrNode.selectedDataSource}") missing`);
            }
            value = null;
        } else {
        ...
        if (attrNode.attributes) {
            ...
            if (attrNode.dataSources) {
                ...
                if (parentKeyRow) {
                    rawResult = getResult(
                        attrNode,
                        attrNode.joinVia || attrNode.primaryDataSource,
                        subContext
                    );
                ...
                if (attrNode.joinVia) {
                    ...
                    keys.forEach((key) => {
                        ...
                        joinViaRows.forEach((joinViaRow) => {
                            ...
                            const childKey = primaryRawResult.parentKey.map((keyAttr) => {
        ...
        if (resource && resource.extensions && resource.extensions.item) {
            resource.extensions.item({
                ...
                getAttribute: attribute =>
                    getAttribute(attribute || [], parentAttrNode),
                ...
                buildItem: (attributes) => {
                    function selectInternal(isSelected) {
                        attributes.forEach((attribute) => {
                            ...
                            attribute.forEach((attrName) => {
                                if (!(attrNode.attributes && attrNode.attributes[attrName])) {
                                    throw new ImplementationError(`Result-Builder (item-extension/buildItem): Unknown attribute "${attribute.join('.')}"`);
                                }
                                attrNode = attrNode.attributes[attrName];

                                if (isSelected && attrNode.selectedInternal) {
                                    throw new ImplementationError(`Result-Builder (item-extension/buildItem): Invalid recursion for "${attribute.join('.')}"`);
                                }
                    ...
                    selectInternal(true);
                    let internalItem = buildItem(parentAttrNode, row, subContext);
...
module.exports = function resultBuilder(api, request, rawResults, resolvedConfig) {
    ...
    if (!resolvedConfig.many) {
        if (rawResult.data.length <= 0) {
            throw new NotFoundError('Item' + (request.id ? ` "${request.id}"` : '') +
