        ...
        if (attrNode.attributes) {
            ...
            if (attrNode.dataSources) {
                ...
                if (parentKeyRow) {
                    rawResult = getResult(
                        attrNode,
                        attrNode.joinVia || attrNode.primaryDataSource, subContext);
        ...
        if (resource && resource.extensions && resource.extensions.item) {
            resource.extensions.item({
                ...
                getResult: (attrNode, dataSourceName) =>
                    getResult(attrNode || parentAttrNode,
                        dataSourceName || (attrNode || parentAttrNode).primaryDataSource, context)
            });
        }
    }
