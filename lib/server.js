...
Server.prototype.run = function (callback) {
    ...
    this.api.init(config, function (err) {
        ...
        self._clusterWorker = new ClusterWorker({
            shutdownTimeout: config.shutdownTimeout,
            log: self.log
        });

        self.api.clusterWorker = self._clusterWorker;
...
Server.prototype._handleRequest = function (httpRequest, httpResponse) {
    ...
    var matches = parsedUrl.pathname.match(/^\/(.+?)\/(.*)$/);
    ...
    if (parsedUrl.query.format) request.format = parsedUrl.query.format;
    if (parsedUrl.query.action) request.action = parsedUrl.query.action;

    if (matches[2]) request.id = matches[2];

    for (var key in parsedUrl.query) {
        if (!request.hasOwnProperty(key)) {
            request[key] = parsedUrl.query[key];
        }
    }

    // TODO: if the URL matches our schema but the resource does not exists,
    // we might need to pass the request to the static handler here.
    this.api.execute(request, function onFloraResponse(err, response) {
        if (err) return self._sendError(err, httpRequest, httpResponse);
...
Server.prototype._sendError = function (err, httpRequest, httpResponse) {
    this._sendResponse({
        meta: {
            statusCode: err.httpStatusCode || errors.FloraError.httpStatusCode
        },
        error: errors.format(err, {exposeErrors: this.api.config.exposeErrors})
