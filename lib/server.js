...
Server.prototype.run = function (callback) {
    ...
    this.api.init(config, function (err) {
        ...
        for (var idx in self._plugins) {
            self.log.debug('Registering plugin' + (self._plugins[idx][0].name ? ' "' + self._plugins[idx][0].name + '"': ''));
    ...
    if (!floraRequest) {
        if (this._staticHandler) {
            return this._staticHandler(httpRequest, httpResponse, function () {
                self._sendError(new NotFoundError('URL "' + httpRequest.url + '" not found (not a valid resource url)'), httpRequest, httpResponse);
            });
        }
        return this._sendError(new NotFoundError('URL "' + httpRequest.url + '" not found (not a valid resource url)'), httpRequest, httpResponse);
    }

    if (!this.api.getResource(floraRequest.resource) && this._staticHandler) {
        return this._staticHandler(httpRequest, httpResponse, function () {
            self._sendError(new NotFoundError('Resource "' + floraRequest.resource + '" not found'), httpRequest, httpResponse);
        });
    }
