...
Server.prototype.run = function (callback) {
    ...
    this.api = new Api();
    this.api._clusterWorker = clusterWorker;
    this.api.init(config, function (err) {
        ...
        // FIXME:
        // This only works if httpServer is set.
        // Maybe refactor clusterWorker to be an EventEmitter.
        self._httpServer.once('close', function () {
            self.close();
        });

        clusterWorker.attach(self._httpServer);
